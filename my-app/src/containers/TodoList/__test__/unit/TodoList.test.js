...
describe('todolist组件', () => {
  ...
  it(' TodoList 应该给 未完成列表 传递 list数据，以及daleteItem 方法,changeStatus 参数', () => {
    const wrapper = shallow(<TodoList />)
    const w = findTestWrapper(wrapper, 'UndoList')
    expect(w.prop('list')).toBeTruthy()
    expect(w.prop('deleteItem')).toBeTruthy()
    expect(w.prop('changeStatus')).toBeTruthy()
    expect(w.prop('handleBlur')).toBeTruthy()
  })
  it(' deleteItem 方法,删除内容', () => {
    ...
    expect(result.current.undoList).toEqual([
      { value: '学习几把毛2', status: 'div' },
    ])
  })
  it(' changeStatus 方法,修改 状态', () => {
    const wrapper = shallow(<TodoList />)
    ...
    act(() => {
      result.current.changeStatus(0)
    })
    const data = result.current.undoList
    expect(data[0]).toEqual({ ...data[0], status: 'input' })
  })
  it(' handleBlur 方法,修改内容', () => {
    const wrapper = shallow(<TodoList />)
    ...
    act(() => {
      result.current.handleBlur(0)
    })
    const data = result.current.undoList
    expect(data[0]).toEqual({ ...data[0], status: 'div' })
  })
  it(' valueChange 方法,修改内容', () => {
    const wrapper = shallow(<TodoList />)
    ...
    act(() => {
      result.current.valueChange(0, '学习')
    })

    const data = result.current.undoList
    expect(data[0]).toEqual({ value: '学习', status: 'input' })
    expect(data[1]).toEqual({ value: '学习几把毛2', status: 'div' })
  })
})
